function cost = cost2D(vec_predictedRgvLocation_en, trix_vec_sensorPointingVec_enu, trix_vec_samplePosition_enu)
    ...
    arguments(Input)
        vec_predictedRgvLocation_en (1,2) double      % A guess of where the RGV is, but only the North and East parts
    ...
    cost = sum(vecnorm(cross(trix_vec_sensorPointingVec_enu,[vec_predictedRgvLocation_en,0]-trix_vec_samplePosition_enu,2),2,2).^2);
end