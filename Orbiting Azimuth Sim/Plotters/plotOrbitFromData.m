function plotOrbitFromData(trix_vec_samplePosition_enu, trix_vec_truePointingVec_enu, trix_vec_sensorPointingVec_enu, bestGuess, params, zoomOnBottom)
    ...
    figure(1)
    plots(1) = scatter3(trix_vec_samplePosition_enu(:,1),trix_vec_samplePosition_enu(:,2),trix_vec_samplePosition_enu(:,3),'filled',DisplayName="True UAS Locations");
    hold on
    grid minor
    axis equal
    % axis vis3d
    ...
    if (zoomOnBottom)
        ...
        zlim([-params.orbitDistance/4 params.orbitDistance/4])
        title_zoom_text = " (Zoomed In)";
    else
    ...
    ylabel("Y [m]")
    zlabel("Z [m]")
    title(sprintf("Orbit Model%s (\\sigma_\\psi=%.0f deg)", title_zoom_text, params.angleStdDeg))
